select pa.user_id, pa.application_id, pas.status_id, rep.response, (EXTRACT(EPOCH FROM sum(age(us.session_end, us.session_start))/3600)::integer) as hours_online
, p.program_name, ap.type_name as application_type_name, pp.name as payment_plan, 
pa.professional_assoc,   
ud.referrer, ud.practice_type, pch.state as home_state, pcw.state as work_state, ud.gender, ud.home_country, ud.work_country, ud.job_title, pc.short_title as curriculum_id, extract(epoch from age(max(uma.end_time), min(uma.start_time)))/3600 as sum
...
join course_enrollment ce on ce.offering_id = co.offering_id and ce.user_id = pa.user_id
...
where p.program_type = 'IHeLp' and p.program_name != 'IHeLp' and ce.status = 'COM' and
pas.status_id in ('W','G','D','I') and ud.is_test_user = false and us.session_start > p.start_date and us.session_start < p.graduation_date
group by pa.user_id, pa.application_id, pas.status_id, p.program_name, ap.type_name, rep.response,
pa.professional_assoc,   
ud.referrer, ud.practice_type, pch.state, pcw.state, ud.gender, ud.home_country, ud.work_country, ud.job_title, pc.curriculum_id, pp.name
